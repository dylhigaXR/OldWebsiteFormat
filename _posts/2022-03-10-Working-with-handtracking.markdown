...
In order to simplify the travel direction I made it so the fireball would always be facing the player using LookAt(). Then, when shot, the ball will move it in its relative backwards direction to simulate it being shot forward... oddly enough.
